...
Приведенную выше трассу вызовов следует читать так:
...
* 94437.50 ns - время моделирования
* 3d8 - адрес, где производился вызов функции
* 6с4 - адрес вызываемой функции (или регистр, где был адрес функции, при вызове фунции по указателю)
* <rumboot_platform_panic> - имя вызываемой функции
...
- Вызовы могут быть вложенными, например:

```
    rumboot_platform_perf("my test1");

        rumboot_platform_perf("sequence1");
        time_consuming_code();
        rumboot_platform_perf(NULL);

        rumboot_platform_perf("sequence2");
        more_time_consuming_code();
        rumboot_platform_perf(NULL);
...
- Для навигации внутрь группы щелкните мышкой на фрагмент диаграммы


## Автоматическое профилирование кода

В режиме автоматического профилирования кода в интерактивную диаграмму сохраняется время выполнения всех функций. При этом учитывается вложенность.

Для включения данного режима необходимо при конфигурации проекта передать опцию
-DRUMBOOT_PROFILE=Yes

Пример:

```
    cmake .. -DRUMBOOT_PROFILE=Yes
    make
    ./runners/run-mytest
    firefox mytest.logs/performance_report.html
```

При просмотре отчета будет доступна кнопка "Functions", которая откроет диаграмму времени выполнения функций.
