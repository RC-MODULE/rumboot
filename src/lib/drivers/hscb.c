...
void hscb_set_descr_in_mem_tx(uint32_t sys_addr, uint32_t src_data_addr, uint32_t len)
{
    ...
    iowrite32( buf, sys_addr + 0x04);

    iowrite32( 0x01000000, sys_addr + 0x08);
    iowrite32( 0, sys_addr + 0x0C);
    iowrite32( 0, sys_addr + 0x10);
    iowrite32( 0, sys_addr + 0x14);

    rumboot_printf("Preparing descriptor TX: MEM[0x%x] = 0x%x\n", sys_addr,     ioread32(sys_addr));
...
void hscb_set_descr_in_mem_rx(uint32_t sys_addr, uint32_t src_data_addr, uint32_t len)
{
    ...
    rumboot_printf("Preparing descriptor RX: MEM[0x%x] = 0x%x\n", sys_addr,     ioread32(sys_addr));
    rumboot_printf("Preparing descriptor RX: MEM[0x%x] = 0x%x\n", sys_addr + 4, ioread32(sys_addr + 4));
}
...
void hscb_enable(uint32_t base_addr)
{
    ...
    hscb_cfg.tx_endian = false;
    hscb_cfg.rx_endian = false;
    hscb_cfg.time_mode = false;
    hscb_cfg.loop_back = false;
    hscb_cfg.en_rmap = false;
    hscb_cfg.en_trim_clk = false;
