...
//ConfigPPE cfg;
 ...
 int nu_ppe_rd_regs(uintptr_t rbase,uintptr_t wbase) {
	  ...
	  int resw0,resw1,resw2,resw3,resw4,resw5,resw6,resw7,resw8,resw9,resw10,resw11,resw12,
		  ...
		  resw37, resw38, resw39, resw40,resw41,resw42,resw43,resw44,resw45, resw46,resw47,resw48,resw49,resw50;
	...
	if ((0x00000000) !=(ioread32(rbase +NU_PPE_RDMA_POINTER))) 	
		{res1 =1;
		rumboot_printf("res1 error\n");}	
		else 
		res1=0;
	if ((0x00000000) !=(ioread32(rbase +NU_PPE_RDMA_OP_ENABLE)& 0x00000001)) 	
		{res2 =1;
		rumboot_printf("res2 error\n");}
		else 
		res2=0;	
	//if ((0x00000000) != (ioread32(rbase +NU_PPE_RDMA_INT_STATUS)& 0x00000171))  //??????  171 !!!!
		if ((0x00000000) !=(ioread32(rbase +NU_PPE_RDMA_OP_ENABLE)& 0x00000001)) 
	{res3 =1;
...
//------------------PPE+WDMA-------------------------------------------
	...
	if ((0x00000000) != (ioread32(wbase + NU_PPE_WDMA_BOX_ST_SIZE_Z)& 0x000001FFF)) 
		...
		resw19=0;	
//	if ((0x00000000) != (ioread32(wbase + NU_PPE_BOX_OFFSET_W_IN)& 0x000001FFF))
//		{resw20 =1;
...
int nu_ppe_wr_rd_regs(uintptr_t rbase,uintptr_t wbase, int32_t data) {
 	  int res;
	  int res2,res3,res4,res5,res6, res7,res8,res9,res10,res11,res12,
		  res13,res14,res15,res16,res17,res18,res19,res20,res21,res22,res23,res24;
	  int resw2,/*resw3,*/resw4,resw5,resw6,resw7,resw8,resw9,resw10,resw11,resw12,
		  resw13,resw14,resw15,resw16,resw17,resw18,resw19,/*resw20,*/resw21,resw22,resw23,resw24,
		  resw25,resw26,resw27,resw28,resw29,resw30,resw31,resw32,resw33,resw34, resw35,resw36,
		  resw37, resw38, resw39, resw40,resw41,resw42,resw43,resw44,resw45, resw46;
	...
	//  int32_t tmp;
	  ...
	  tmpr = data;
		...
		iowrite32(tmpr, rbase + NU_PPE_RDMA_OP_ENABLE);
		if ((tmpr & 0x000000001 )  !=(ioread32(rbase +NU_PPE_RDMA_OP_ENABLE)& 0x00000001)) 	
		{res2 =1;
		rumboot_printf("res2 error\n");}
		else 
		res2=0;	
...
//------------------PPE+WDMA-------------------------------------------
		...
		iowrite32(tmpr, rbase + NU_PPE_WDMA_INT_RESET);	
	if ((tmpr & 0x00000171) !=(ioread32(wbase +NU_PPE_WDMA_INT_RESET)& 0x00000171)) 	
		{resw4 =1;
		rumboot_printf("resw4 error\n");}
		else 
		resw4=0;
	
		iowrite32(tmpr, rbase + NU_PPE_WDMA_INT_SET);	
	if ((tmpr & 0x00000171) != (ioread32(wbase + NU_PPE_WDMA_INT_SET)& 0x00000171))
		{resw5 =1;
		rumboot_printf("resw5 error\n");}
		else 
		resw5=0;
	
		iowrite32(tmpr, rbase + NU_PPE_WDMA_INT_MASK);	
	if ((tmpr & 0x00000171) != (ioread32(wbase + NU_PPE_WDMA_INT_MASK)& 0x00000171))
		...
		iowrite32(tmpr, rbase + NU_PPE_WDMA_BASE_ADDR);	
	...
	if ((tmpr & 0x0007FFFF) != (ioread32(wbase + NU_PPE_KERNEL)& 0x0007FFFF))
		{resw33 =1;
		rumboot_printf("resw33 error\n");}
		else 
		resw33=0;
	...
	if ((tmpr & 0x00077777) != (ioread32(wbase + NU_PPE_PADDING)& 0x00077777)) 	
		{resw36 =1;
		rumboot_printf("resw36 error\n");}	
		else 
		resw36=0;
	...
	if ((tmpr & 0x0007FFFF) !=(ioread32(wbase +NU_PPE_PADDING_VALUE_7)& 0x0007FFFF)) 	
		...
		iowrite32(tmpr,wbase + NU_PPE_INF_NUM_IN);
	if ((tmpr & 0x0007FFFF) !=ioread32(wbase + NU_PPE_INF_NUM_IN)) 	
		{resw44 =1;
		rumboot_printf("resw44 error\n");}	
		else 
		resw44=0;
	
		iowrite32(tmpr,wbase + NU_PPE_NAN_NUM_IN);	
	if (tmpr !=ioread32(wbase + NU_PPE_NAN_NUM_IN)) 	
		{resw45 =1;
		rumboot_printf("resw45 error\n");}	
		else 
		resw45=0;
	
		iowrite32(tmpr,wbase + NU_PPE_NAN_NUM_OUT);	
	if (tmpr !=ioread32(wbase + NU_PPE_NAN_NUM_OUT)) 	
		{resw46 =1;
		rumboot_printf("resw46 error\n");}	
		else 
		resw46=0;		
     ...
     resr = 	res2 || res3 || res4 || res5 || res6 || res7 || res8 ||
				res9 || res10 || res11 || res12 || res13 || res14 || res15  || res16 || res17 ||
				res18 || res19 || res20 || res21  || res22 || res23  || res24 ;
	  resw = 	resw2 || resw4 || resw5 || resw6 || resw7 || resw8 || resw9 ||
				resw10 || resw11 || resw12 || resw13 || resw14 || resw15 || resw16 || resw17 ||
				resw18 || resw19 || /*resw20 || */ resw21 || resw22 || resw23 || resw24 || resw25 ||
				resw26 || resw27 || resw28 || resw29 || resw30 || resw31 || resw32 || resw33 ||
				resw34 ||resw35 || resw36 ||resw37 ||resw38 || resw39 || resw40 || resw41 ||
				resw42 || resw43  || resw44 ||resw45 ||resw46 ; 
