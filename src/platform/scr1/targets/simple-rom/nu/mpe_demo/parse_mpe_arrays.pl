...
sub parse_and_write_hex_word {
  ...
  for($pos=$word_len-2;$pos>=0;$pos=$pos-2) {  # From The Last Byte Of String To The First Byte
    ...
    print $fh pack("C", $byte);                # Write The Byte To File
  }
...
while(defined($cline = <STDIN>)) {
  ...
  if($ft_process == 1) {
    if($cline =~ m/0x([\w_]+)/) {
      $word_hex = $1;
      parse_and_write_hex_word($word_hex, $ft_fh, 1024/8);
      $ft_size += 1024/8;
    }
  }
  elsif($wt_process == 1) {
    if($cline =~ m/0x([\w_]+)/) {
      $word_hex = $1;
      parse_and_write_hex_word($word_hex, $wt_fh, 1024/8);
      $wt_size += 1024/8;
    }
  }
  elsif($et_process == 1) {
    if($cline =~ m/0x([\w_]+)/) {
      $word_hex = $1;
      parse_and_write_hex_word($word_hex, $et_fh, 512/8);
      $et_size += 512/8;
    }
  }
  elsif($cmd_process == 1) {
    if($cline =~ m/0x([\w_]+)/) {   # Hex Representation
      $word_hex = $1;
      parse_and_write_hex_word($word_hex, $cmd_fh, 64/8);
      $cmd_size += 64/8;
    }
...
if($et_size > 0) { close($et_fh); write_metrics_file($et_size,$et_file); }
