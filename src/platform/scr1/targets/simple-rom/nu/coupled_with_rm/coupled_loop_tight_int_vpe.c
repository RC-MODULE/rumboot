...
#ifdef VPE_CUBE_CMPL
	nu_vpe_set_int_mask(MY_VPE_REGS_BASE, &cfg); 	
#endif     
...
if ((i== (iterations-1)) | (i == 0)){
		...
		nu_vpe_pause_next_cntx(MY_VPE_REGS_BASE, &cfg);		
		nu_vpe_soft_reset(MY_VPE_REGS_BASE, &cfg);	
		nu_vpe_dev_pause_resume(MY_VPE_REGS_BASE, &cfg);
	...
	nu_vpe_wait_int_pause_cntx_appl(MY_VPE_REGS_BASE, &cfg);

	}	  
else
	{nu_vpe_dev_resume(MY_VPE_REGS_BASE, &cfg);
	nu_vpe_wait_int_cntx_appl(MY_VPE_REGS_BASE, &cfg);}
