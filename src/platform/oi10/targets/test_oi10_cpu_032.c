...
static void check_system_hang() {
    rumboot_putstring("Check SYSTEM HANG");
}
...
int main () {
    if(rumboot_platform_runtime_info->persistent[START_STATUS] != STARTED ) {
        rumboot_platform_runtime_info->persistent[START_STATUS] = STARTED;
    ...
    rumboot_putstring("ITrace HANG value:");
...
ASM_TEXT(       beq             rumboot_CI_hdr_restore_CR               )
ASM_TEXT(       mtcr            r3                                      )
ASM_TEXT(       load_const      r3, DCR_ITRACE_BASE+ITC0_ITAI           )
ASM_TEXT(       load_const      r4, ITRACE_HANG_ADDR                    )
ASM_TEXT(       mtdcrx          r3, r4                                  )   // set itrace address
...
ASM_TEXT(       b               rumboot_CI_hdr_restore_r3_r4            )
ASM_TEXT(   rumboot_CI_hdr_restore_CR:                                  )
ASM_TEXT(       mtcr            r3                                      )
ASM_TEXT(   rumboot_CI_hdr_restore_r3_r4:                               )
ASM_TEXT(       mfspr           r4, SPR_SPRG8                           )
ASM_TEXT(       mfspr           r3, SPR_USPRG0                          )
ASM_TEXT(       b               rumboot_CI_hdr                          )
...
static void init_handlers() {
    ...
    rumboot_irq_set_handler( tbl, O_SYSTEM_HUNG, RUMBOOT_IRQ_LEVEL | RUMBOOT_IRQ_HIGH, handler_system_hung_dummy, ( void* ) 0);
...
static void check_no_hang_on_load() {
    rumboot_putstring("Check no SYSTEM HANG on load");
