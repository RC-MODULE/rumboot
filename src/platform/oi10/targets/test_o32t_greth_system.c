...
//    uint32_t j=0;
    ...
    rx_descriptor_data_ = (greth_descr_t*) rumboot_malloc_from_named_heap_aligned(DESC_HEAP_NAME, N_DESC * sizeof(greth_descr_t), 1024);
    ...
    for (i = 0; i < N_DESC; i++){
        test_data_dst[i] = (uint8_t* )rumboot_malloc_from_named_heap_aligned(DST_HEAP_NAME, GRETH_TEST_DATA_LEN_BYTES, 64);
    }
...
// rx desc config
    ...
    for (i = 0; i < N_DESC; i++){
        ...
        iowrite32((last<<GRETH_RX_DESCR_IE) |
                  ...
                  (1   <<GRETH_RX_DESCR_EN) |
                  GRETH_TEST_DATA_LEN_BYTES ,rumboot_virt_to_dma(rx_descriptor_data_) + i*8);
                
        iowrite32(rumboot_virt_to_dma(test_data_dst[i]),rumboot_virt_to_dma(rx_descriptor_data_) + 4 + i*8);
    }    
...
// tx desc config 
    for (i = 0; i < N_DESC; i++){
        ...
        iowrite32((last<<GRETH_TX_DESCR_WR) |
                  (1   <<GRETH_TX_DESCR_EN) |
                  GRETH_TEST_DATA_LEN_BYTES ,rumboot_virt_to_dma(tx_descriptor_data_) + i*8);
                  
        iowrite32(rumboot_virt_to_dma(test_data_src),rumboot_virt_to_dma(tx_descriptor_data_) + 4 + i*8);    
    } 
...
void start_transfer_wait_irq()
{
    ...
    iowrite32( 0xffffffff, RX_GRETH_BASE + STATUS);
    // setting descriptor pointer
    iowrite32(rumboot_virt_to_dma(rx_descriptor_data_), RX_GRETH_BASE + RECEIVER_DESCR_PTR);
    iowrite32(rumboot_virt_to_dma(tx_descriptor_data_), TX_GRETH_BASE + TRANSMIT_DESCR_PTR);
    //Start greth
    iowrite32(0x9a00009a, RX_GRETH_BASE);
    iowrite32(0x9a000091, TX_GRETH_BASE);
