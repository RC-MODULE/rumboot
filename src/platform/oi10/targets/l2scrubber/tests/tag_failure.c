...
int main()
{
    ...
    do_memfill32(l2scrubber_tests_get_cachable_area(), 256*1024, 0, 1);
    asm("msync");
    asm("isync"); 

    for (int j=0; j < 256*1024; j = j + 8) {
        ioread64(j);
    }

    /* This magic bullet will fix everything */
// /    l2_scrubber_enable_pattern_scrubbing(scr, 0x5a5a5a5a5a5a5a5a);
    ...
    int i;
    for (i=1024; i<2048; i++) {
        l2_inject_tag_ecc_fault(DCR_L2C_BASE, i, 1);
//        l2_inject_tag_fault(DCR_L2C_BASE, 512, 31);
    }

    rumboot_printf("l2c: readback 1\n");
    ...
    l2_dump_tags(500,50);
    rumboot_printf("l2c: readback 2| %d\n", scr->corrected_tag_errors);

    for (int j=0; j < 256*1024; j = j + 8) {
        ioread64(j);
    }
    rumboot_printf("l2c: readback 3\n");
