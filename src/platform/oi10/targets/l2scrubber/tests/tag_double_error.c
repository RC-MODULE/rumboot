...
int main()
{
    ...
    //struct l2_scrubber *scr = l2_scrubber_create(DCR_L2C_BASE, 0);
    struct l2c_mem_layout layout;
    l2c_get_mem_layout(DCR_L2C_BASE, &layout);
//***********************************************************************************************************************************************************************************************************
    rumboot_printf("First do_memfill32 start\n");
    do_memfill32(l2scrubber_tests_get_cachable_area(), 128, 0, 1);
    asm("msync");
    asm("isync"); 
    rumboot_printf("First do_memfill32 complete\n");
    rumboot_printf("First control read (no tag double error)\n");
    for (int j=0; j < 512; j = j + 8) {
        rumboot_printf("data[%d] = %d \n",j,ioread64(j));
    }
    rumboot_printf("l2c: Init complete\n");
    rumboot_irq_sei();
    l2c_l2_write(DCR_L2C_BASE, L2C_L2MCKEN, 0);
    rumboot_printf("l2c: Switch-off MACHINE_CHECK\n");

    rumboot_printf("Injection error in STATUS\n");
    l2_inject_tag_fault(DCR_L2C_BASE, 1, 31);
    l2_inject_tag_fault(DCR_L2C_BASE, 1, 30);
    //----------------
    //rumboot_printf("Injection error in MSB bit of TAG\n");
    //l2_inject_tag_fault(DCR_L2C_BASE, 1, 28);
    //rumboot_printf("Injection error in LSB bit of TAG\n");
    //l2_inject_tag_fault(DCR_L2C_BASE, 1, 3);
    ////----------------
    //rumboot_printf("Injection error in MSB bit of ECC\n");
    //l2_inject_tag_ecc_fault(DCR_L2C_BASE, 1, 7);
    //rumboot_printf("Injection error in LSB bit of ECC\n");
    //l2_inject_tag_ecc_fault(DCR_L2C_BASE, 1, 1);
    //rumboot_printf("Finish injection of error - X\n");
          
    rumboot_printf("Second control read (with tag double error)\n");
    for (int j=0; j < 512; j = j + 8) {
        rumboot_printf("data[%d] = %d \n",j,ioread64(j));
    }
//***********************************************************************************************************************************************************************************************************    
    //rumboot_printf("Second do_memfill32 start\n");
    //do_memfill32(l2scrubber_tests_get_cachable_area(), 128, 0, 1);
    //asm("msync");
    //asm("isync"); 
    //rumboot_printf("Second do_memfill32 complete\n");
    //
    //   for (int j=0; j < 512; j = j + 8) {
    //    ioread64(j);
    //}
