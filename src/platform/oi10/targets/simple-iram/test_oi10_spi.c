...
#define S_GPIO_BASE                           0x3C066000

//Interrupt flags
extern uint32_t volatile SPI_HANDLED;
...
void NON_CR_INTERRUPT_HANDLER(void)
{
    ...
    if(source_vector == 100){
      MEM(0x3C061014) = 0x0;  //Mask interrupts
      MEM(0x3C061020) = 0x3;  //Reset interrupts
      SPI_HANDLED = true;
    }else if(source_vector == 73){
      MEM(0x3C062014) = 0x0;  //Mask interrupts
      MEM(0x3C062020) = 0x3;  //Reset interrupts
      SPI_HANDLED = true;
    }
    END_NONCR_INT_P0;
}

uint32_t wait_spi_int_handled(unsigned timeout)
{
    unsigned i = 0;
    while(!SPI_HANDLED && (i++<timeout)){;}
...
int read_eeprom_status (unsigned spi_apb_base)
{
  ...
  MEM(spi_apb_base+0x008) = 0x05; //write data to SPI - command read status
  MEM(spi_apb_base+0x008) = 0xff; //write data to SPI - data staff
  ...
  return (MEM(spi_apb_base+0x008)); //read data from rx fifo
}
