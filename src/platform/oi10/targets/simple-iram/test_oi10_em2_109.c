...
#define NUM_ELEM 256
///src
static uint32_t* em2_cs0_sram_out = (uint32_t*)(SRAM0_BASE);
static uint32_t* em2_cs1_nor_out = (uint32_t*)(NOR_BASE);
static uint32_t* em0_cs2_sdram_out = (uint32_t*)(SDRAM_BASE);
static uint32_t* em2_cs1_nor_out_3000 = (uint32_t*)(NOR_BASE);
///dst
static uint32_t* em2_cs0_sram_in = (uint32_t*)(SRAM0_BASE);
static uint32_t* em2_cs0_sram_in_2000 = (uint32_t*)(SRAM0_BASE);
static uint32_t* em2_cs0_sdram_in_4000 = (uint32_t*)(SDRAM_BASE);
static uint32_t* em2_cs2_sdram_in_8000 = (uint32_t*)(SDRAM_BASE);
...
static void init_data(uint32_t* addr, uint32_t value)
{
    ...
    {
        iowrite32(value, (uint32_t)(addr + j));
    }
}
...
int main (void)
{
    ...
    dma_info0.source_adr = rumboot_virt_to_dma((uint32_t*)em2_cs0_sram_out + 0x100);
    dma_info0.dest_adr = rumboot_virt_to_dma((uint32_t*)em2_cs0_sram_in + 0x100 + NUM_ELEM*sizeof(uint32_t));
    ...
    dma_info0.transfer_width = tr_width_halfword;
    dma_info0.rw_transfer_size = rw_tr_size_2q;
    dma_info0.count = size_for_dma;

    dma2plb6_mcpy(&dma_info0);
    init_data(em2_cs0_sram_out, 0xaaaa);

    dma2plb6_setup_info dma_info1;
    channel_status status1 = {};
    dma_info1.base_addr = DCR_DMAPLB6_BASE;
    dma_info1.source_adr = rumboot_virt_to_dma((uint32_t*)em2_cs1_nor_out + 0x200);
    dma_info1.dest_adr = rumboot_virt_to_dma((uint32_t*)em2_cs0_sram_in_2000 + 0x2000 + NUM_ELEM*sizeof(uint32_t));
    ...
    init_data(em2_cs1_nor_out, 0xbbbb);

    dma2plb6_setup_info dma_info2;
    channel_status status2 = {};
    dma_info2.base_addr = DCR_DMAPLB6_BASE;
    dma_info2.source_adr = rumboot_virt_to_dma((uint32_t*)em0_cs2_sdram_out + 0x300);
    dma_info2.dest_adr = rumboot_virt_to_dma((uint32_t*)em2_cs0_sdram_in_4000 + 0x4000 + NUM_ELEM*sizeof(uint32_t));
    ...
    dma_info2.channel = channel0;
    dma_info2.transfer_width = tr_width_halfword;
    dma_info2.rw_transfer_size = rw_tr_size_2q;
    dma_info2.count = size_for_dma/8;

    dma2plb6_mcpy(&dma_info2);
    init_data(em0_cs2_sdram_out, 0xcccc);

    dma2plb6_setup_info dma_info3;
    channel_status status3 = {};
    dma_info3.base_addr = DCR_DMAPLB6_BASE;
    dma_info3.source_adr = rumboot_virt_to_dma((uint32_t*)em0_cs2_sdram_out + 0x300);
    dma_info3.dest_adr = rumboot_virt_to_dma((uint32_t*)em2_cs2_sdram_in_8000 + 0x8000 + NUM_ELEM*sizeof(uint32_t));
    ...
    dma_info3.channel = channel0;
    dma_info3.transfer_width = tr_width_halfword;
    dma_info3.rw_transfer_size = rw_tr_size_2q;
    dma_info3.count = size_for_dma/8;

    dma2plb6_mcpy(&dma_info3);
    init_data(em2_cs1_nor_out_3000, 0xdddd);
