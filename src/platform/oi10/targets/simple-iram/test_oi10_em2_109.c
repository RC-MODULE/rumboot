...
static bool check_data1(uint32_t addr_dst, uint32_t addr_src)
{
    rumboot_printf("Check data...\n");
    rumboot_printf("Count of data elements: %d \n", NUM_ELEM);
    register uint32_t i = 0;
    ...
    {
        ///em2.cs0_in->im2.cs0_out
        if (ioread32(addr_dst + 4* i) != ioread32(addr_src + 4 * i))
        {
          //  rumboot_printf ("Error in output data 1, addr = %d\n", ioread32(addr_src[i]));
          //  rumboot_printf("Expected data = %d\n", ioread32(addr_src[i]));
          //  rumboot_printf("Actual data = %d\n", ioread32(addr_dst[i]));
...
//  rumboot_printf ("Start wait...\n");
   ...
   // if (wait_dma2plb6_mcpy(&dma_info0, &status0) && wait_dma2plb6_mcpy(&dma_info1, &status1) &&
    ...
    /*
    if((wait_dma2plb6_mcpy(&dma_info0, &status0) == false && check_data() == false) && (wait_dma2plb6_mcpy(&dma_info1, &status1) == false && check_data() == false)
            && (wait_dma2plb6_mcpy(&dma_info2, &status2) == false && check_data() == false)
             && (wait_dma2plb6_mcpy(&dma_info3, &status3) == false && check_data() == false))
    {
