...
volatile bool emi_single_irq_raised                 = false;
volatile bool emi_double_irq_raised                 = false;
volatile bool mclfir_recoverable_error_irq_raised   = false;
volatile bool mclfir_unrecoverable_error_irq_raised = false;
...
static void irq_handler( int irq, void *arg ) {
    ...
    switch (irq) {
        ...
        case MCLFIR_REC_INT:
            ...
            mclfir_recoverable_error_irq_raised = true;
        ...
        case MCLFIR_UNREC_INT:
            ...
            mclfir_unrecoverable_error_irq_raised = true;
...
bool check_data()
{
    ...
    TEST_WAIT_ASSERT(emi_single_irq_raised == true, ERROR_WAIT_TIMEOUT, "SRAM0: waiting single error timeout.");

    emi_single_irq_raised = false;
    ...
    TEST_WAIT_ASSERT(mclfir_recoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SRAM0: waiting recoverable error timeout.");

    mclfir_recoverable_error_irq_raised = false;
    ...
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000001, "SRAM0: expected recoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_double_irq_raised == true, ERROR_WAIT_TIMEOUT, "SRAM0: waiting double error timeout.");

    emi_double_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000002, "SRAM0: expected double error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0002);


    TEST_WAIT_ASSERT(mclfir_unrecoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SRAM0: waiting unrecoverable error timeout.");

    mclfir_unrecoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000002, "SRAM0: Expected unrecoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_single_irq_raised == true, ERROR_WAIT_TIMEOUT, "SSRAM: waiting single error timeout.");

    emi_single_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000004, "SSRAM: expected single error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0004);


    TEST_WAIT_ASSERT(mclfir_recoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SSRAM: waiting recoverable error timeout.");

    mclfir_recoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000001, "SSRAM: expected recoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_double_irq_raised == true, ERROR_WAIT_TIMEOUT, "SSRAM: waiting double error timeout.");

    emi_double_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000008, "SSRAM: Expected double error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0008);


    TEST_WAIT_ASSERT(mclfir_unrecoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SSRAM: waiting unrecoverable error timeout.");

    mclfir_unrecoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000002, "SSRAM: expected unrecoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_single_irq_raised == true, ERROR_WAIT_TIMEOUT, "SDRAM: waiting single error timeout.");

    emi_single_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000010, "SDRAM: expected single error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0010);


    TEST_WAIT_ASSERT(mclfir_recoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SDRAM: waiting recoverable error timeout.");

    mclfir_recoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000001, "SDRAM: expected recoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_double_irq_raised == true, ERROR_WAIT_TIMEOUT, "SDRAM: waiting double error timeout.");

    emi_double_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000020, "SDRAM: expected double error");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0020);


    TEST_WAIT_ASSERT(mclfir_unrecoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "SDRAM: waiting unrecoverable error timeout.");

    mclfir_unrecoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000002, "SDRAM: expected unrecoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_single_irq_raised == true, ERROR_WAIT_TIMEOUT, "NOR: waiting single error timeout.");

    emi_single_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000040, "NOR: expected single error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0040);


    TEST_WAIT_ASSERT(mclfir_recoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "NOR: waiting recoverable error timeout.");

    mclfir_recoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000001, "NOR: expected recoverable error.");
    ...
    TEST_WAIT_ASSERT(emi_double_irq_raised == true, ERROR_WAIT_TIMEOUT, "NOR: waiting double error timeout.");

    emi_double_irq_raised = false;

    TEST_ASSERT(dcr_read(DCR_EM2_EMI_BASE + EMI_IRR) == 0x000200, "NOR: expected double error.");
    dcr_write(DCR_EM2_EMI_BASE + EMI_IRR_RST, 0x0200);


    TEST_WAIT_ASSERT(mclfir_unrecoverable_error_irq_raised == true, ERROR_WAIT_TIMEOUT, "NOR: waiting unrecoverable error timeout.");

    mclfir_unrecoverable_error_irq_raised = false;

    rumboot_printf("MCLFIR_MC_ERR0 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR0));
    rumboot_printf("MCLFIR_MC_ERR1 = 0x%x\n", dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_ERR1));
    TEST_ASSERT(dcr_read(DCR_EM2_MCLFIR_BASE + MCLFIR_MC_LFIR) == 0x00000002, "NOR: expected unrecoverable error.");
