...
//    check_dcu_lru_array();
    rumboot_memfill32((void*)(SRAM0_BASE ), mem_size_instr + 0x40, 0, 0x2);

    msync();
    for(int i = 0; i < mem_size_instr; i += 0x2000)
    {
        dcbtls(0, i);
    }
    msync();
    for(int i = 0; i < mem_size_instr; i += 0x2000)
    {
        uint32_t data = dcread(i);
        rumboot_printf("data = 0x%x\n", data);
        rumboot_printf("dcdbtrh = 0x%x\n", spr_read(SPR_DCDBTRH));
        rumboot_printf("dcdbtrl = 0x%x\n", spr_read(SPR_DCDBTRL));
