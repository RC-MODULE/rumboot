...
const uint32_t loop_counter = 512;//4;//
...
uint32_t data[DATA_ARRAY_SIZE] __attribute__((section(".EM0.data"))) = {0};


static inline uint32_t __attribute__((section(".EM0.text"))) convert_itrace_address(uint32_t address)
{
    return ((address << 1) | (address >> 31));
}

static inline uint32_t __attribute__((section("EM0.text"))) get_stop_address(uint32_t address)
{
...
static int __attribute__((section(".EM0.text"))) test_itrace(ITRACE_CONDITION_TYPE condition_type,
...
/*
    ...
    uint32_t ppcX = mpic128_get_current_proc_num(MPICx_BASE) == Processor0 ? 0 : 1;
    msync();
#endif
    ccr0 |= 0x80000000;
    ccr0 &= 0xFFFF7FFF;
    spr_write(SPR_CCR0, ccr0);
    ...
    if(atomic_test_and_set((uint32_t) &(active_core_flags[0]),
            ...
            (ppcX|0x1110) ))
    {
        rumboot_printf("Started preparation for ITRACE enabling\n");
#endif
*/
        //rumboot_printf("Disable\n");
        itrace_disable(DCR_ITRACE_BASE);
        itrace_clear_status(DCR_ITRACE_BASE);
...
//        }
        ...
        if((trigger_mode == ITRACE_TRIGGER_MODE_BANK_ON_TRIGGER))
            last_address[1] = ((( itrace_status >> ITRACE_TS_ITA2W_i ) & 1) ?
                ((((ITRACE_BUFFER_SIZE/2) - 1) | ((1<<ITRACE_TLA2_VALID_i) & last_address[1])) ^ 0x80000000)
                : last_address[1]);
        last_address[0] = ((( itrace_status >> ITRACE_TS_ITA1W_i ) & 1) ?
                (((ITRACE_BUFFER_SIZE/
        ...
        rumboot_printf("last_address[0] == 0x%x", last_address[0]);
        rumboot_printf("last_address[1] == 0x%x", last_address[1]);

        for(i = 0; i<2; ++i){
            stop_address[i] = get_stop_address(last_address[i]);
            rumboot_printf("stop_address[i] == 0x%x", stop_address[i]);
        }

#ifndef TEST_MPW_CPU_027_SRAM_MIRROR_FAST_DEBUG_NO_DCR
...
//#else //TEST_MPW_CPU_027_SRAM_MIRROR_FAST_DEBUG_NO_DCR
        for (j = 0; j <
            (((trigger_mode == ITRACE_TRIGGER_MODE_BANK_ON_TRIGGER)
                    && ((1<<ITRACE_TLA2_VALID_i) & last_address[1]))
                    ? 2 : 1)
                    ; j++)
        {
            address = ((j & 1) << (DATA_ARRAY_ADDRESS_BITS - 2));
            i = ((j & 1) << (DATA_ARRAY_ADDRESS_BITS - 1 ));
            rumboot_printf("j == %d", j);
            rumboot_printf("i == %d", i);
            rumboot_printf("address == 0x%x", address);
            do
            {
...
//
                rumboot_printf("data[%d] = 0x%x", i, data[i]);
//                rumboot_printf("data_address = 0x%x", (uint32_t)(&data[i]));
...
//                trace_hex(i+1);
//
                rumboot_printf("data[%d] = 0x%x", i+1, data[i+1]);
//                rumboot_printf("data_address = 0x%x", (uint32_t)(&data[i+1]));
...
//                }
    ...
    //    rumboot_printf("Store data address = 0x%x", IM0_ADDR_CELL);

    //    trace_dump((uint32_t)&data, 1024);

        test_event(EVENT_ITRACE_CHECK_DATA);
#endif //TEST_MPW_CPU_027_SRAM_MIRROR_FAST_DEBUG_NO_DCR
