...
static bool read_and_check (uint32_t base_addr, uint32_t size_b, uint32_t data_inc)
{
    for (uint32_t i = 0; i < (size_b/sizeof(uint64_t)); i++)
    {
        ...
        uint64_t volatile rdata = ioread64(addr);
        ...
        if ( (((uint32_t)(rdata >> 32)) != GET_EXP_DATA(addr, data_inc)) || ( ((uint32_t)(rdata & 0xFFFFFFFF)) != GET_EXP_DATA((addr+0x4), data_inc)) )
        {
            ...
            rumboot_printf("data_l = %x\n", (uint32_t) GET_EXP_DATA((addr+0x4), data_inc) );
        ...
        //l2c get data
        uint64_t l2c_data = 0;
        if(l2c_arracc_data_read_by_way(DCR_L2C_BASE, (uint32_t) (phys_addr >> 32), (uint32_t) (phys_addr), cache_way, &l2c_data) == false)
        {
...
bool __attribute__((section(".text.test"))) cache_testing_function()
{
    ...
    for (uint32_t i = 0, d = GET_EXP_DATA((START_ADDR + DATA_SIZE), PTRN_INC_2); i < (L2C_LINE_SIZE/4); i++, d += PTRN_INC_2)
        iowrite32(d, (START_ADDR + DATA_SIZE) + 4*i);
    msync();
