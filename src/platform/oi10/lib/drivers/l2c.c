...
{
    uint32_t tmp;
    tmp = l2c_l2_read(l2c_dcr_base, L2C_L2INTEN);
    SET_BITS_BY_MASK(tmp, mask);
    l2c_l2_write(l2c_dcr_base, L2C_L2INTEN, tmp);
}

void l2c_global_mck_enable(uint32_t l2c_dcr_base, L2MCKEN_bits_t mask)
{
    uint32_t tmp;
    tmp = l2c_l2_read(l2c_dcr_base, L2C_L2MCKEN);
    SET_BITS_BY_MASK(tmp, mask);
    l2c_l2_write(l2c_dcr_base, L2C_L2MCKEN, tmp);
}
...
void l2c_global_mck_disable(uint32_t l2c_dcr_base, L2MCKEN_bits_t mask)
{
    ...
    l2c_l2_write(l2c_dcr_base, L2C_L2MCKEN, tmp);
    TEST_ASSERT(l2c_l2_read(l2c_dcr_base, L2C_L2MCKEN)==tmp, "Error setting L2MCKEN");
}

void l2c_enable_interrupt(uint32_t l2c_dcr_base, uint32_t l2c_int_reg_index, uint32_t mask)
{
    uint32_t tmp;
    tmp = l2c_l2_read(l2c_dcr_base, l2c_int_reg_index);
    SET_BITS_BY_MASK(tmp, mask);
    l2c_l2_write(l2c_dcr_base, l2c_int_reg_index, tmp);
}

void l2c_pmu_set_CE_bit(uint32_t pmu_dcr_base, uint32_t dcr_index)
{
    uint32_t tmp;
    tmp = l2c_pmu_read(pmu_dcr_base, dcr_index);
    l2c_pmu_write(pmu_dcr_base, dcr_index, tmp | PMULCX_CE);
}
...
void l2c_pmu_enable_interrupt(uint32_t pmu_dcr_base, uint32_t dcr_index, uint32_t mask)
{
    uint32_t tmp;
    tmp = l2c_pmu_read(pmu_dcr_base, dcr_index);
    l2c_pmu_write(pmu_dcr_base, dcr_index, tmp | mask);
}

void pmu_disable_interrupt(uint32_t pmu_dcr_base, uint32_t dcr_index, uint32_t mask)
{
    uint32_t tmp;
    tmp = l2c_pmu_read(pmu_dcr_base, dcr_index);
    CLEAR_BITS_BY_MASK(tmp, mask);
    l2c_pmu_write(pmu_dcr_base, dcr_index, tmp);
}

void l2c_pmu_clear_interrupt(uint32_t pmu_dcr_base)
{
    uint32_t tmp;
    tmp = l2c_pmu_read(pmu_dcr_base, L2C_PMUIS0);
    l2c_pmu_write(pmu_dcr_base, L2C_PMUIS0, tmp);
}
...
bool l2c_arracc_tag_info_read_by_way (uint32_t base, uint32_t ext_phys_addr, uint32_t phys_addr, int32_t cache_way, volatile uint32_t* tag_info)
{
	...
	l2c_l2_write( base, L2C_L2ARRACCCTL, ( l2arraccctl ) | L2C_ARRACCCTL_REQUEST_FIELD |
	...
	while(((l2c_l2_read( base, L2C_L2ARRACCCTL ) & L2C_ARRACCCTL_READ_REQUEST_COMP_FIELD) == 0) && (indx < L2C_TIMEOUT))
		indx++;
