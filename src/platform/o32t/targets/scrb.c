...
bool check_if_scrubbing_done(void *arg) {
    ...
    if (a->im3_enable) {
        result &=(scrb_get_number_im3_errors(SCRB_BASE)>0);
    }    
    return result;
}
...
bool test_scrb( uint32_t structure ) {
    ...
    iowrite32(0, SCRB_BASE+SCRB_REG_FCNT1);
    iowrite32(0, SCRB_BASE+SCRB_REG_FCNT2);
    iowrite32(0, SCRB_BASE+SCRB_REG_FCNT3);
    ...
    scrb_start_scrubber(SCRB_BASE, (stru->im3_enable)<<2|(stru->im2_enable)<<1|(stru->im1_enable));
    ...
    //// This is done to prevent another scrubbing cycle b4 it can be stoped
    //if(stru->mode==0x1) {
    //    
    //    // Wait until all 3 EN signals are set
    //    // Its about 3*stru->timer_value*8
    //    // Its done by reading a reg
    //    // This way we can ensure that this cycle wont be 
    //    // deleted from program by compiler
    //    ioread32(SCRB_BASE);
    //    //rumboot_printf("count %d times", 3*(stru->timer_value+8));
    //    //for (int i = 0; i<3*(stru->timer_value+8); i++){
    //    //    ioread32(SCRB_BASE);
    //    //}        
    //    
    //    
    //    rumboot_printf("Stoping scrubbing for MODE = 1 \n");
    //    scrb_stop_scrubbing_im1(SCRB_BASE);
    //    scrb_stop_scrubbing_im2(SCRB_BASE);
    //    scrb_stop_scrubbing_im3(SCRB_BASE);
    //} 
    ...
    int errors_fcnt_im3 = scrb_get_number_im3_errors(SCRB_BASE);
    ...
    int errors_im3_expected = stru->im3_enable ? 20 : 0;//%stru->im2_errors;
    int errors_all_expected = errors_im1_expected+errors_im2_expected+errors_im3_expected;//%stru->all_errors;
    ...
    // If more then 1 mem is beeing scrubbed its imposible to count IR1 properly
    // So this is checked if only 1 mem enabled
    if ((!(stru->im1_enable) & ((stru->im2_enable)^(stru->im3_enable))) | ((stru->im1_enable) &(!(stru->im2_enable)) &(stru->im3_enable))){
    ...
    if (errors_fcnt_im3!=errors_im3_expected){
        rumboot_printf("Wrong number of im3 fcnt errors - got %d, expected - %d", errors_fcnt_im3, errors_im3_expected);
        result = false;
    }
    ...
    if(stru->dont_mask_all){
        // If more then 1 mem is beeing scrubbed its imposible to count IR1 properly
        // Expected value of allcnt anly can be evaluated if we now proper IR1 value
        // So this is checked if only 1 mem enabled
        if ((!(stru->im1_enable) & ((stru->im2_enable)^(stru->im3_enable))) | ((stru->im1_enable) &(!(stru->im2_enable)) &(stru->im3_enable))){
...
static struct scrb_test_config in_new[ ] = {
    // 0
    {
        .test_number    = 0,
        .im1_enable     = 1,
