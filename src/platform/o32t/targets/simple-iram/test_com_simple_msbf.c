...
void prepare_arrays( uint32_t ** src, uint32_t ** dst ) {
	  ...
	  *src = rumboot_malloc_from_named_heap_aligned( COM_SRC_HEAP, sizeof(uint64_t)*ARR_SIZE, sizeof(uint64_t) );
      *dst = rumboot_malloc_from_named_heap_aligned( COM_DST_HEAP, sizeof(uint64_t)*ARR_SIZE,sizeof(uint64_t) );

//rumboot_printf("ARR_SIZE=%d\n",(ARR_SIZE>>1));
	for( uint64_t i = 0; i < ARR_SIZE; i++ )
        (*src)[ i ] = i + ( (i+1) << 8 ) + ( (i+2) << 16 ) + ( (i+3) << 24 ) + ( (i+4) << 32)  + ( (i+5) << 40)  + ( (i+6) << 48)  + ( (i+7) << 56);
