...
void scrb_set_work_mode(uint32_t base_addr, uint32_t mode_value)
{
    ...
    old_value = old_value & (~(1<<4));
    uint32_t new_value = old_value|mode_value<<4;
...
void scrb_set_priority(uint32_t base_addr, uint32_t priority_value)
{
    ...
    old_value = old_value & (~(1<<5));
    uint32_t new_value = old_value|priority_value<<5;
    iowrite32(new_value, base_addr+SCRB_REG_CR);
}
...
void scrb_start_scrubber(uint32_t base_addr, uint32_t start_im3_im2_im1_bits)
{
    ...
    old_value = old_value & (~(7));
    uint32_t new_value = old_value|start_im3_im2_im1_bits;
    iowrite32(new_value, base_addr+SCRB_REG_CR);
}
...
{
    return ioread32(base_addr+SCRB_REG_FCNT1);
}

int scrb_get_number_im2_errors(uint32_t base_addr)
{
    return ioread32(base_addr+SCRB_REG_FCNT2);
}

int scrb_get_number_im3_errors(uint32_t base_addr)
{
    return ioread32(base_addr+SCRB_REG_FCNT3);
}
...
//            is_finished=is_finished|0x2;
//        }
//    }
//    if(im3_enable>0){
//        if (scrb_get_number_im3_errors(base_addr)>0){
//            is_finished=is_finished|0x4;
//        }
//    }
//    return is_finished;
//}