...
static bool test_mgeth_frame_xfer(uint32_t eth_send_num)
{
	...
	iowrite32(0x00000010 << (eth_send_num * 8), SGMII_PHY + 0x112C);
	iowrite32(0x00080000, SGMII_PHY + 0x1134 + eth_send_num * 4);
	...
	rumboot_printf("Waiting of sending of output packet (timeout expected)...\n");

	ret = mgeth_wait_transfer_complete(chan_o, 20);
	if (ret != 1)
	{
		rumboot_printf("ERROR: Frame sended? No timeout (ret = %d)!\n", ret);
		rumboot_free((void *)frame_o);
		rumboot_free((void *)frame_i);
		return false;
	}

	rumboot_printf("Output packet not sended (timeout).\n");

	rumboot_printf("Waiting of receiving of input packet (timeout expected)...\n");

	ret = mgeth_wait_transfer_complete(chan_i, 20);
	if (ret != 1)
	{
		rumboot_printf("ERROR: Frame received? No timeout (ret = %d)!\n", ret);
		rumboot_free((void *)frame_o);
		rumboot_free((void *)frame_i);
		return false;
	}

	rumboot_printf("Input packet not received (timeout).\n");
	...
	iowrite32(0x0, SGMII_PHY + 0x112C);
