...
void gp_timer_create_event ()
{
    ...
    rdata = ioread32 (GLOBAL_TIMERS + TMR_5_STATE);
    iowrite32 (rdata | 0x2, GLOBAL_TIMERS + TMR_5_STATE);
}
...
uint32_t main ()
{
    ...
    rumboot_printf("    mkio1_events_timestamps_test\n");
    ...
    {
        if (mkio_events_timestamps_test ((uint32_t) (&data_src), (uint32_t) (&data_dst), DATA_SIZE, MKIO1_BASE, MKIO0_BASE) != 0)
            return -3;
