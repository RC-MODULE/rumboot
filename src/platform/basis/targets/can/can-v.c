...
bool run_can_loopback(uint32_t can_base_addr) {
    int test_state = 1;
...
bool run_can_fifo(uint32_t can_base_addr) {
    int test_state = 3;
...
bool run_can_full(uint32_t can_base_addr) {
    int test_state = 4;
    int ret = CCAN_FULL(can_base_addr, 0, 0, test_state);
    if (ret) { rumboot_printf("ret %08x test failed\n", ret); }
    return 0;
}

bool run_can_mask_filter(uint32_t can_base_addr) {
    int test_state = 5;
...
int main()
{
    ...
    //int ret = run_can_loopback(can0_base_addr); // 0xcan_base_addr
    ...
    int ret = run_can_fifo(can0_base_addr); // 0xcan_base_addr
