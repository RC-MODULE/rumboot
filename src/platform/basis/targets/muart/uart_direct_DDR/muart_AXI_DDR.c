...
//    char *testing_heaps[t_heap] = {"IM0", "DDR0", "DDR1", "PCIE-IM0"};
    char *testing_heaps[t_heap] = {"DDR0", "DDR1"};

    rumboot_printf("Test 2.2 - 2.4 \n");
    rumboot_printf("Check access MUART to DDR3_0,DDR3_1 memories\n");
    ...
    for (heap_id = 0; heap_id < heap_cnt; heap_id++) {
        for (i = 0; i < t_heap; i++) {
  rumboot_printf("test  %s   %s\n",heaps_name[heap_id],testing_heaps[i]);
            if ((strcmp(heaps_name[heap_id],testing_heaps[i])) == 0) break;
        }
  rumboot_printf("After cmp  i=%d heap_id=%d\n",i,heap_id);
        if(i >= t_heap) continue;
        rumboot_printf("test_alloc_mem  %s  i=%d\n",heaps_name[heap_id],i);
    ...
    if (cnt_test == 0) {
        rumboot_printf("UNTESTED: Needed heaps are not found\n");
        res_fail++;
    }
    if (res_fail == 0) {
