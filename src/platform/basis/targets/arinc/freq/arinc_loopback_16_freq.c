...
void arinc_init (uint32_t arinc_base_addr, uint32_t tx_freq, uint32_t rx_freq ){
	...
	//uint32_t   tx_freq = 0xe30141f2;//freq =100KHz
	//uint32_t   rx_freq = 0xe01f41f2;//freq =100KHz
//-------------------------------------------------------------------------------------
...
//iowrite32(i,(arinc_base_addr + RNUM_RX +(4*i +64))); 		// odd receiver buffer ch number
	...
	iowrite32(tx_freq,(arinc_base_addr + FREQ_TX + i*4));			//freq TX 100Mhz
	iowrite32(rx_freq,(arinc_base_addr + FREQ_RX + i*4));			//freq RX 100MHz
	}
...
int main()
{
	...
	arinc_init(ARINC_BASE, TX_FREQ, RX_FREQ);
