...
// 
//  Test includes:
//  - 8 ARINC429 transmitters and receivers delivery through the external loopback
//	- 8  receivers accept short arrays awithout checking labels 
...
int arinc_status_check8(uint32_t status_x_tx, uint32_t status_x_rx) {
    uint32_t tmp_r= -1;
    int cnt=0;
    uint32_t tmp=0; 
    int i;
...
int main() {
    ...
    tmp = arinc_status_check8(STAT_E_TX, STAT_E_RX);
    if (tmp == TEST_ERROR)  {
