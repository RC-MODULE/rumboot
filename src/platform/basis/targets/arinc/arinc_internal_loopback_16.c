...
//-------------------------------------------------------------------------------------	
	...
	iowrite32((i + 16),(ARINC_BASE + SELF_RX  + i*4));   		// make internal loop
	...
	iowrite32(0xffff,(ARINC_BASE + SW_SIG_RX));   			    	// wait delayed switch after the receiver start	
	}
...
int main()
{
	...
	uint32_t *rrg = (uint32_t *) rx_mem;
	memcpy(rrg, 0x0,sizeof(tx_array32)); 
