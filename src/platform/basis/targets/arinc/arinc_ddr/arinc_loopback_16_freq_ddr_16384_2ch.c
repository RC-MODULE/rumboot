...
int arinc_status_check16(uint32_t status_x_tx, uint32_t status_x_rx) {
	...
	rumboot_printf("ARINC CH=0x%x\n", i); 	
      // while (tmp != 0x00000004) {
		while (tmp != 0x00004000) {  
	tmp = ioread32(ARINC_BASE + STAT_E_TX + i*4);
	tmp = 0x07FFFFFF & tmp;
		if (++cnt == 100 * ARINC_ATTEMPT) {
...
int main()
{
	...
	uint32_t *tx_mem = rumboot_malloc_from_heap_aligned(heap_0, 600000, 4);
	uint32_t *rx_mem = rumboot_malloc_from_heap_aligned(heap_1,	600000, 4);	
...
for (i = 0; i< 32768 ; i++)	
	...
	if (tmp == ARINC_OK) {
	rumboot_printf("ARINC data comparison OK\n");
	for (i = 0; i< 2 ; i++)
	{		
