...
int i2c_init( uint32_t scl_freq){
	uint32_t tmp_ctrl;
	...
	iowrite32((0x8),( I2C_BASE + I2C_NUMBER));//number of bytes
...
for (i = 0; i< (byte_array_size); i++)
   ...
   // rumboot_printf("i2c_addr_h =0x%x\n",i2c_addr_h);
    ...
    iowrite32((i2c_addr /*+ I2C_RD */),(i2c_base_addr + I2C_TRANSMIT));
	...
	{iowrite32((I2C_BASE + I2C_RD ),(I2C_BASE + I2C_TRANSMIT));
	result = ioread32(i2c_base_addr + I2C_RECEIVE);
   // compare results
    if (result  != data[i]) {
	  rumboot_printf("I2C test ERROR!\n");
...
//interrupt for last READ, interrupt for last write
  // size_array = sizeof(check_array32) / 4;
	...
	iowrite32((0x1),(i2c_base_addr + I2C_CTRL)); //reset START bit in CR_REG
