...
unsigned read_data; 
  ...
  //SDIO_SPI = false;
/*  iowrite32(0xc7,GSPI_BASE + 0x000); //8-bit data
  iowrite32(0x03,GSPI_BASE + 0x004); //turn on controller, loop operation
  iowrite32(0x02,GSPI_BASE + 0x010); //clock prescale
  iowrite32(0x00,GSPI_BASE + 0x014); //interrupt masks - mask all
  iowrite32(0x03,GSPI_BASE + 0x024); //enable DMA
 ...
 //GSPI loop test - dma exchenge
  iowrite32(0xc,GSPI_BASE + 0x0d8); //interrupt masks - unmask axiw
  iowrite32((DMA_SDIO_SRC_IM0),(GSPI_BASE + 0x0a4));//DMAR start address 
  iowrite32((DMA_SDIO_SRC_IM0 + 31),(GSPI_BASE + 0x0a8));//DMAR end address
  ...
  iowrite32((DMA_SDIO_SRC_IM0 + 0x100),(GSPI_BASE + 0x090));//DMAW start address
  iowrite32((DMA_SDIO_SRC_IM0 + 0x100 + 31),(GSPI_BASE + 0x094));//DMAW end address
...
int sdio_dma_tr (unsigned src_addr, unsigned dst_addr)
{
  ...
  iowrite32(0x000020f1,SDIO_BASE + 0x028);//ARLEN=16,ARSIZE=2(4bytes)
  //if (!wait_sdio_int_handled(1000)) {trace_msg("SDIO interrupt timeout\n"); return TEST_ERROR;}
...
read_data=ioread32 (SDIO_BASE+ 0x048);
 ...
 iowrite32(0x00020f01,SDIO_BASE + 0x038);
 // if (!wait_sdio_int_handled(1000)) {trace_msg("SDIO interrupt timeout\n"); return TEST_ERROR;}
 read_data=ioread32 (SDIO_BASE+ 0x28);
  while (!(read_data&0x00080000))
      {read_data=ioread32 (SDIO_BASE+ 0x028);}
