//-----------------------------------------------------------------------------
//  This program is for checking DDR1 SDRAM correct work
//  
//  Test includes:
//    - 
//    - 
...
//-----------------------------
    ...
    for (uint32_t i = 0; i < (sizeof (src_test_array) >> 3); i++)
    {
        if (((uint32_t *) EMI1_BASE) [i] != ((uint32_t *) src_test_array) [i])
        {
            ret = -1;
            rumboot_printf ("  ERROR: stability check FAILED.\n");
            rumboot_printf ("    etalon 0x%08x    read 0x%08x\n", ((uint32_t *) src_test_array) [i], ((uint32_t *) EMI1_BASE) [i]);
        }
    }
#endif
...
//-----------------------------
    ...
    ddr_check_status (DDR1_BASE);
    
#ifdef TEST_FAST
        rumboot_printf ("  start fast post test  %d\n", rep_cnt);
        tmp = memory_post_test(EMI1_BASE, 0x1FEC0000, pattern, &otherpattern, debug_log, 0);
#elif TEST_NORMAL
        rumboot_printf ("  start normal post test  %d\n", rep_cnt);
        tmp = memory_regions_post_test(EMI1_BASE, 0x1FEC0000, pattern, &otherpattern, debug_log, 1);
#else
        rumboot_printf ("  start maximum post test  %d\n", rep_cnt);
        tmp = memory_post_test(EMI1_BASE, 0x1FEC0000, pattern, &otherpattern, debug_log, 1);
#endif
        if (tmp) {
