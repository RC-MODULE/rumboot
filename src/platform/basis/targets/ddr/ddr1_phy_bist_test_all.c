//-----------------------------------------------------------------------------
//  This program is for checking DDR1 in BIST mode
...
int main ()
{
    ...
    //-------------------------
    // iowrite32 (ioread32 (DDR1_BASE + DENALI_CTL_00) & 0xFFFFFF00, DDR1_BASE + DENALI_CTL_00);
    // delay_cycle (1000000);
    
    if ((ioread32 (DDR1_BASE + DENALI_CTL_00) & 0x1) == 0)
    ...
    if (ddr_bist            (DDR1_BASE) != 0)
        return -2;
    if (ddr_bist_ac         (DDR1_BASE) != 0)
        return -3;
    if (ddr_bist_clk        (DDR1_BASE) != 0)
        return -4;
    //---------------------------------------------------------
    //  Dll test doesnt pass always on first run
    //  results of DLL test dont allow to pass all other tests on second run
    //-------------------------
    // if (ddr_bist_dll_data   (DDR1_BASE) != 0)
        // return -5;
    // if (ddr_bist_dll_ac     (DDR1_BASE) != 0)
        // return -6;
