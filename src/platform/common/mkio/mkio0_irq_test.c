...
uint32_t mkio_write_to_rt_with_irq (uint32_t data_src, uint32_t data_dst, uint32_t size, uint32_t bc_base_address, uint32_t rt_base_address)
{
    ...
    rumboot_printf("    data_src %x\n",                 rumboot_virt_to_dma (&data_src                 ));
    rumboot_printf("    data_dst %x\n",                 rumboot_virt_to_dma (&data_dst                 ));
    rumboot_printf("    mkio_bc_descriptor %x\n",       rumboot_virt_to_dma (&mkio_bc_descriptor       ));
    rumboot_printf("    mkio_rt_subaddress_table %x\n", rumboot_virt_to_dma (&mkio_rt_subaddress_table ));
    rumboot_printf("    mkio_rt_rx_descriptor %x\n",    rumboot_virt_to_dma (&mkio_rt_rx_descriptor    ));
    rumboot_printf("    mkio_irq_ring_buffer %x\n",     rumboot_virt_to_dma (&mkio_irq_ring_buffer     ));
    ...
    mkio_rt_subaddress_table.sa1_rx_descriptor_pointer = rumboot_virt_to_dma(&mkio_rt_rx_descriptor) ;
    ...
    iowrite32 (rumboot_virt_to_dma(&mkio_irq_ring_buffer), bc_base_address + BCRD);

    mkio_rt_start_schedule (rt_base_address, rumboot_virt_to_dma(&mkio_rt_subaddress_table));
    mkio_bc_start_schedule (bc_base_address, rumboot_virt_to_dma(&mkio_bc_descriptor      ));
...
//------------------------------------------------------------------------
//  MKIO0 IRQ handler function.
//    Set irq_flag main program to acknowledge irq recieving
//------------------------------------------------------------------------
static void MKIO0_IRQ_handler (int irq, void *arg)
{
    uint32_t rdata;

	rumboot_printf("MKIO0_IRQ_handler start \n");
    irq_flag = 0x5ACCE551;
    ...
    {
        iowrite32 ((1 << i), MKIO0_BASE + IRQE);
    }

    rdata = ioread32 (MKIO0_BASE + IRQ);
    rumboot_printf("MKIO0 IRQ reg value: 0x%x\n", rdata);
    iowrite32 (rdata, MKIO0_BASE + IRQ);
}
...
uint32_t main ()
{
    ...
    rumboot_irq_set_handler(tbl, MKIO0_IRQ,  RUMBOOT_IRQ_LEVEL | RUMBOOT_IRQ_HIGH, MKIO0_IRQ_handler, NULL);
    /* Activate the table */
    rumboot_irq_table_activate(tbl);
    rumboot_irq_enable(MKIO0_IRQ);
    rumboot_irq_sei();
    ...
    if (mkio_write_to_rt_with_irq (rumboot_virt_to_dma(data_src), rumboot_virt_to_dma(data_dst), DATA_SIZE, MKIO0_BASE, MKIO1_BASE) != 0)
        return -3;
